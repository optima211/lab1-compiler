...
void bubble(long* Mas, long len) {
	...
	long long time2 = clock();   //конечный замер времени
...
int main()
{
	const long len = 35000;     //вычисленное эмпирически значение длины массива
	...
	for (int i = 0; i < 10; i++) {
		...
		cout << "Test " << (i + 1) << ":" << endl;
		initialData(Mas1, Mas2, len);   //инициализация начальных данных
		bubble(Mas1, len);              //алгоритм пузырьковой сортировки
		Shell(Mas2, len);               //алгоритм сортировки Шелла
		cout << endl;
	}
